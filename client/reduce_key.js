...
export function KeyReduction(commandEncoder, splats)
{
    {
        let pipeline = GetPipeline1();
        const passEncoder = commandEncoder.beginComputePass();
        let count = splats.splatBuffer.getSplatCount();
        for (let i=0; i<splats.dKey.length-1; i++)
        {
            count = Math.floor((count + workgroup_size_2x - 1)/workgroup_size_2x);
            passEncoder.setPipeline(pipeline);
            passEncoder.setBindGroup(0, splats.bind_group_key_reduction[i]);
            passEncoder.dispatchWorkgroups(count, 1,1); 
        }
        passEncoder.end();
    }

    commandEncoder.copyBufferToBuffer(splats.dKey[splats.dKey.length-1], 0, splats.dKeyMinMax, 0, 4);
    ...
    commandEncoder.copyBufferToBuffer(splats.dKey[splats.dKey.length-1], 0, splats.dKeyMinMax, 4, 4);
